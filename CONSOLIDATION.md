...
| 9 | le lap manuel du 29/08 apparaît (19:47 · 4,03 km · 4'55/km · FC 177) | ❌ **toujours non vérifié — vraie lacune de données, en cours de traitement hors bricolage**. `runActivityDetail` (sync Strava) ne renseignait jamais `isManual` en persistant les tours — corrigé pour tout futur import (`isManual = lap.split == null`), mais rétroactivement, sur les 137 activités actuelles, **aucun tour n'a `splitIndex` NULL**. Tentative de re-synchronisation ciblée de cette activité (05/09) : échec avant même l'appel Strava — jeton de rafraîchissement refusé (400), reconnexion du compte nécessaire depuis Réglages. Base sauvegardée avant tentative (`prisma/dev.db.avant-resync-lap-2908-20260905-054125.bak`), aucune donnée touchée. **Décision explicite : ne pas basculer `isManual` à la main sur un split kilométrique automatique** — ce serait fabriquer une donnée qui n'existe pas (contrevient à R1, « ne jamais inventer une donnée »). Suite : l'utilisateur reconnecte Strava, la tâche déjà en file (`activity_detail`, stravaId `19945370426`) sera retraitée automatiquement à la prochaine synchronisation. Si Strava ne renvoie toujours pas de lap manuel après reconnexion, la piste suivante est le fichier FIT natif de la montre COROS — pas une correction en base. **Repère retrouvable en attendant** : le bloc d'effort du 29/08 correspond aux splits automatiques #5 à #8 de l'activité "Test de seuil" (`cmte6rflc00021wcr0b9262ac`) — 3,95 km en 19:36 à 4'58/km, FC 174-183. |
