# Trainer HUB — suivi d'entraînement (projet autonome)

> **Ce dossier n'a aucun rapport avec le site Alartic**, qui vit dans son
> propre dossier `alartic/` à la racine du dépôt, avec ses propres règles
> (Astro, Rust/Axum, zéro ressource tierce, Decap, Codeberg, jalons du
> cahier des charges) — elles **ne s'appliquent pas ici**. Ce projet a ses
> propres conventions, son propre `package.json` et son propre lockfile ; il
> est volontairement autonome, sans workspace npm partagé avec Alartic.
...
- **aucune authentification** : application pensée pour un usage local ou sur
  un réseau de confiance. Si elle doit un jour être exposée sur Internet, la
  protection d'accès se met en place devant (reverse proxy, VPN), pas dans le
  code applicatif.
...
- L'état OAuth Strava et le secret de la tâche planifiée sont vérifiés en
  temps constant (`safeEqual`) — ce sont les seules vérifications d'accès qui
  subsistent, l'application elle-même n'a pas d'authentification (cf.
  ci-dessus).
...
`feat(trainer-hub):`, `fix(trainer-hub):`, `test(trainer-hub):`,
`docs(trainer-hub):`.
