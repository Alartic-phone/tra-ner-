# Trainer HUB — suivi d'entraînement course à pied
...
> Ce dossier est un projet autonome à la racine du dépôt. Il n'a **aucun
> rapport** avec le site Alartic (`alartic/`) : les règles de son `CLAUDE.md`
> ne s'y appliquent pas. Voir `trainer-hub/CLAUDE.md`.
...
# 5. Amorcer le cycle de postes et le profil
npm run db:seed
...
L'application écoute sur <http://localhost:3000>, sans authentification —
pensée pour un usage local ou sur un réseau de confiance. Si elle doit être
exposée sur Internet, mettre une protection d'accès devant (reverse proxy
avec auth, VPN).
...
2. Remplir le formulaire de création :
   - **Application Name** : `Trainer HUB` (ou ce que vous voulez).
   - **Category** : `Training`.
   - **Club** : laisser vide.
   ...
   - **Authorization Callback Domain** : `localhost` en local, sinon **le
     ...
     (par exemple `trainerhub.mondomaine.fr`). C'est l'erreur la plus
     fréquente.
...
npm run backup -- /media/disque/trainer-hub.db    # destination choisie
npm run restore -- backups/trainer-hub-2026-08-24T10-00-00.db
```
...
```cron
30 3 * * * cd /chemin/trainer-hub && /usr/bin/npm run backup >> /var/log/trainer-hub-backup.log 2>&1
```
...
L'application écoute sur `127.0.0.1:3000` uniquement, sans authentification.
Pour l'exposer au-delà du réseau local, passer par un reverse proxy qui
termine TLS **et** protège l'accès (auth basique, VPN…) — l'application
elle-même ne filtre rien. La base vit dans le volume `trainer-hub-data`.
...
3. Sur Vercel, déclarer les variables d'environnement de `.env.example`
   ...
   Anthropic). Vercel expose l'application sur Internet par défaut : ajouter
   une protection d'accès devant (Vercel Authentication, ou un reverse proxy),
   l'application elle-même n'en a pas.
